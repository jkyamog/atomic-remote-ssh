...
test("all five core modules are erasable and importable under node --test", async () => {
  const ssh = await import("../atomic-remote-ssh/ssh.ts");
  ...
  const targets = await import("../atomic-remote-ssh/targets.ts");
  assert.equal(typeof targets.createTargetRegistry, "function");
  assert.equal(typeof targets.sidOf, "function");

  const paths = await import("../atomic-remote-ssh/paths.ts");
  assert.equal(typeof paths.toRemotePath, "function");

  const commands = await import("../atomic-remote-ssh/commands.ts");
  ...
  const ops = await import("../atomic-remote-ssh/ops.ts");
